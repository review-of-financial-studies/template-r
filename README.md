...
This repository serves as a template for reproducible R projects, addressing three common challenges:

- **Project execution**: Scripts should never use `setwd()` to change the working directory, as this breaks portability and will not work on other machines. Instead, R should always be run from the project's root directory. The simplest approach is to place `main.R` in the root and open the folder directly in [RStudio](https://posit.co/download/rstudio-desktop/), [Positron](https://positron.posit.co/), or [Visual Studio Code](https://code.visualstudio.com/docs/languages/r).
- **Portability**: Relative file paths using forward slashes (`"/"`) allow the code to run on any system without modification. Single backslashes (`"\"`) are treated as an escape character in R and should not be replaced by double backslashes (`"\\"`).
- **Dependencies**: The `renv.lock` file documents how the [R environment](https://rstudio.github.io/renv/index.html) is configured.
...
- **R** (code was last run with version 4.5.2)
  - With packages `renv`, `dplyr`, `arrow`, `lubridate`, `ggplot2`, and `fixest`
  
The packages are recorded in the `renv.lock` file. To restore the environment, open the project folder in your IDE, run `renv::restore()`, and restart your R session. 
...
All results are automatically saved to the appropriate directories (`results/figures/` and `results/tables/`).

### License for Code

The code is licensed under a Modified BSD License license. See LICENSE file for details. 
...
1. Ensure you have R 4.5.2 or higher installed
2. Ensure you have an active internet connection (required to download data from FRED)
3. Open R with the working directory in the replication package root folder
4. Run `renv::restore()`
...
|----------------|---------|-------------|-------------|------|
| Figure 1 | code/analyze_data.R | 3-23 | results/figures/unemployment_inflation.png | Scatterplot with fitted line |
| Table 1 | code/analyze_data.R | 25-28 | results/tables/phillips_regression.tex | Regression results |
